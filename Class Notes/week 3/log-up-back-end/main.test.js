...
describe('isValidEmail', () => {
  it('returns whether the email ends with a persevere domain', () => {
    ...
    assert(isValidEmail(validEmail1)).toBe(true);
    assert(isValidEmail(validEmail2)).toBe(true);
    assert(isValidEmail(invalidEmail1)).toBe(false);
    assert(isValidEmail(invalidEmail2)).toBe(false);
  })
  ...
  it('requires there be at least one character before the @ symbol', () => {
    ...
    assert(isValidEmail(validEmail1)).toBe(true);
    assert(isValidEmail(validEmail2)).toBe(true);
    assert(isValidEmail(validEmail3)).toBe(true);
    assert(isValidEmail(validEmail4)).toBe(true);
    assert(isValidEmail(invalidEmail1)).toBe(false);
    assert(isValidEmail(invalidEmail2)).toBe(false);
  })
})
...
describe('isValidPassword', () => {
  it('returns whether the password is at least 8 characters long', () => {
    ...
    assert(isValidPassword(validPassword1)).toBe(true);
    assert(isValidPassword(validPassword2)).toBe(true);
    assert(isValidPassword(invalidPassword1)).toBe(false);
    assert(isValidPassword(invalidPassword2)).toBe(false);
  })
...
describe('isRegisteredUser', () => {
  it('returns whether or not the given email is in the "database"', () => {
    ...
    assert(isRegisteredUser(registeredUser1)).toBe(true);
    assert(isRegisteredUser(registeredUser2)).toBe(true);
    assert(isRegisteredUser(unregisteredUser1)).toBe(false);
    assert(isRegisteredUser(unregisteredUser2)).toBe(false);
  })
})

describe('passwordMatches', () => {
  it('returns whether or not the given email/password combination goes with a current user', () => {
    assert(passwordMatches(user1, password1)).toBe(true);
    assert(passwordMatches(user2, password2)).toBe(true);
    assert(passwordMatches(user3, password3)).toBe(true);
    assert(passwordMatches(user1, 'NOT my password')).toBe(false);
    assert(passwordMatches(user2, 'nor HERS')).toBe(false);
    assert(passwordMatches(user3, 'and not his EITHER')).toBe(false);
    assert(passwordMatches(user1, password2)).toBe(false);
    assert(passwordMatches(user2, password3)).toBe(false);
    assert(passwordMatches(user3, password1)).toBe(false);
  })
})
